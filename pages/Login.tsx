...
const Login: React.FC = () => {
  ...
  useEffect(() => {
    const savedEmail = localStorage.getItem('eco_remember_email');
    if (savedEmail) {
      setEmail(savedEmail);
      setRememberMe(true);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (rememberMe) {
        localStorage.setItem('eco_remember_email', email);
      } else {
        localStorage.removeItem('eco_remember_email');
      }
      ...
      <div className="max-w-[440px] w-full space-y-8 bg-white p-10 rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.04)] border border-slate-100 relative z-10 animate-in fade-in zoom-in-95 duration-700">
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="flex items-center justify-between px-1">
            ...
            <Link to="/forgot-password" title="Forgot Password" className="text-xs font-bold text-emerald-600 hover:text-emerald-700 transition-colors">
              Forgot Key?
            </Link>
          </div>
